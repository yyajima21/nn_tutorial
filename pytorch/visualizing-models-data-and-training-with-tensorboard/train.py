...
def plot_classes_preds(net, images, labels):
    ...
    for idx in np.arange(4):
        ...
        utils.matplotlib_imshow(images[idx], one_channel=True)
        ax.set_title("{0}, {1:.1f}%\n(label: {2})".format(
            classes[preds[idx]],
            probs[idx] * 100.0,
            classes[labels[idx]]),
...
def test(net, device, testloader, writer):
    ...
    for i in range(len(classes)):
        writer = add_pr_curve_tensorboard(i, test_probs, test_preds, writer)
    return writer
